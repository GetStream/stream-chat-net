...
namespace StreamChatTests
{
    ...
    public class ChannelClientTests : TestBase
    {
        ...
        [Test]
        public async Task TestChannelTruncateWithOptionsAsync()
        {
            // Send multiple messages to ensure there is content to truncate
            await _messageClient.SendMessageAsync(_channel.Type, _channel.Id, _user1.Id, "First message");
            await _messageClient.SendMessageAsync(_channel.Type, _channel.Id, _user1.Id, "Second message");

            // Create a system message for truncation
            var systemMessage = new MessageRequest
            {
                Text = "Channel has been truncated",
            ...
            var truncateOptions = new TruncateOptions
            {
                ...
                UserId = _user1.Id,
                MemberIds = new List<string> { _user1.Id, _user2.Id },
            };

            await _channelClient.TruncateAsync(_channel.Type, _channel.Id, truncateOptions);

            var afterTruncateChannel = await _channelClient.GetOrCreateAsync(_channel.Type, _channel.Id,
                new ChannelGetRequest
                {
                    State = true,
                    MessagesPagination = new MessagePaginationParams { Limit = 100 },
                });

            afterTruncateChannel.Messages.Should().NotBeEmpty();
            var systemMsg = afterTruncateChannel.Messages[0];
            systemMsg.Type.Should().Be(MessageType.System);
            systemMsg.Text.Should().Be("Channel has been truncated");
            systemMsg.User.Id.Should().Be(_user1.Id);
        }

        [Test]
        public async Task TestChannelTruncateWithHardDeleteAsync()
        {
            await _messageClient.SendMessageAsync(_channel.Type, _channel.Id, _user1.Id, "Message to be hard deleted");

            await _channelClient.TruncateAsync(_channel.Type, _channel.Id, new TruncateOptions
            {
                HardDelete = true,
                UserId = _user1.Id,
            });
        ...
        [Test]
        public async Task TestChannelTruncateWithSystemMessageAsync()
        {
            await _messageClient.SendMessageAsync(_channel.Type, _channel.Id, _user1.Id, "Message to be truncated");

            var systemMessage = new MessageRequest
            {
                Text = "Custom truncation message",
                Type = MessageRequestType.System,
            };
            ...
            var afterTruncateChannel = await _channelClient.GetOrCreateAsync(_channel.Type, _channel.Id,
                new ChannelGetRequest { State = true });
            afterTruncateChannel.Messages.Should().HaveCount(1);
            afterTruncateChannel.Messages[0].Type.Should().Be(MessageType.System);
            afterTruncateChannel.Messages[0].Text.Should().Be("Custom truncation message");
        }

        [Test]
        public async Task TestChannelTruncateWithSkipPushAsync()
        {
            await _messageClient.SendMessageAsync(_channel.Type, _channel.Id, _user1.Id, "Message to be truncated");
        ...
        [Test]
        public async Task TestChannelTruncateWithTruncatedAtAsync()
        {
            await _messageClient.SendMessageAsync(_channel.Type, _channel.Id, _user1.Id, "Message to be truncated");

            var truncatedAt = DateTimeOffset.UtcNow;
            await _channelClient.TruncateAsync(_channel.Type, _channel.Id, new TruncateOptions
            {
        ...
        [Test]
        public async Task TestChannelTruncateWithUserAsync()
        {
            await _messageClient.SendMessageAsync(_channel.Type, _channel.Id, _user1.Id, "Message to be truncated");

            await _channelClient.TruncateAsync(_channel.Type, _channel.Id, new TruncateOptions { UserId = _user1.Id });
        ...
        [Test]
        public async Task TestChannelTruncateWithMemberIdsAsync()
        {
            await _messageClient.SendMessageAsync(_channel.Type, _channel.Id, _user1.Id, "Message to be truncated");

            var memberIds = new List<string> { _user1.Id, _user2.Id };
            await _channelClient.TruncateAsync(_channel.Type, _channel.Id, new TruncateOptions
            {
                MemberIds = memberIds,
                UserId = _user1.Id,
