...
```csharp
await messageClient.SendMessageAsync(channel.Type, channel.Id, "user-id", "Hello, world!");
```
...
```csharp
await messageClient.SendMessageAsync(_channel.Type, _channel.Id, new MessageRequest
{
...
```csharp
var updated = await messageClient.UpdateMessageAsync(new MessageRequest
{
  Id = message.Id,
...
> [!WARNING]
> Deleting a message does not delete its file attachments. See [deleting attachments](/chat/docs/dotnet-csharp/file_uploads/#deleting-files-and-images/) for more information.
...
// Hard delete
await messageClient.DeleteMessageAsync(message.Id, hardDelete: true);
```
