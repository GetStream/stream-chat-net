...
> [!NOTE]
> In legacy permission system users can never access users nor channels from other teams. In [Permissions V2](/chat/docs/dotnet-csharp/chat_permission_policies/) it is possible to alter this behavior using multi-tenant permissions.
...
var response = await client.UpsertAsync(userWithTeamRole);
```
...
In tables below you will find default permission grants for builtin roles that designed for multi-tenant applications. They are useful for [multi-tenant applications](/chat/docs/dotnet-csharp/multi_tenant_chat/) only.
