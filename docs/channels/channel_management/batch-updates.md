...
var members = new[]
{
new ChannelBatchMemberRequest { UserId = "user-123" },
};

var resp = await updater.AddMembersAsync(filter, members);
...
var resp = await updater.UpdateDataAsync(filter, data);
```
...
```csharp
var taskResponse = await taskClient.GetTaskStatusAsync(response.TaskId);
```
