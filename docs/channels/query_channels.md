...
> [!NOTE]
> For supported query operators, see [Query Syntax Operators](/chat/docs/dotnet-csharp/query_syntax_operators/).
...
For the full list of supported query operators, see [Query Syntax Operators](/chat/docs/dotnet-csharp/query_syntax_operators/).
...
var channels = await channelClient.QueryChannelsAsync(
    QueryChannelsOptions.Default
        .WithPredefinedFilter("user_messaging_channels")
        .WithFilterValue("user_id", userId)
        .WithSortBy(new SortParameter { Field = "last_message_at", Direction = SortDirection.Descending })
        .WithLimit(20)
);
```
...
var channels = await channelClient.QueryChannelsAsync(
    QueryChannelsOptions.Default
        .WithPredefinedFilter("user_messaging_channels")
        .WithFilterValue("user_id", "user123")
        .WithLimit(20)
);
```
