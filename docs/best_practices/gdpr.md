...
GDPR gives EU citizens the right to request access to their information and the right to have access to this information in a portable format. Stream Chat covers this requirement with the [Export User](/chat/docs/dotnet-csharp/exporting_channels/) method.

This method can only be used with server-side authentication.

```csharp
var exportedUser = await ServerClient.Users.Export(userID);
```
...
- [Deleting a batch of users](/chat/docs/dotnet-csharp/update_users/#deleting-many-users/)

- [Deleting a batch of channels](/chat/docs/dotnet-csharp/channel_delete/#deleting-many-channels/)
