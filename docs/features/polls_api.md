...
var poll = await channel.CreatePollAsync(new StreamCreatePollRequest { Name = "Where should we host our next event?" });
await channel.SendNewMessageAsync(new StreamSendMessageRequest { Text = "Vote now!", PollId = poll.Id });

// Vote on a poll
await poll.CastVoteAsync(messageId, optionId);
